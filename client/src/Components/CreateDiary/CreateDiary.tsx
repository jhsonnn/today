  ...
  const handleClickEditDiary = async () => {
    ...
    try {
      ...
      if ("serviceWorker" in navigator && "PushManager" in window) {
        ...
        registration?.showNotification("💌 새로운 비밀일기가 저장되었습니다.", {
          body: `미리보기: ${title}`,
          icon: "../../../public/icon/icon.jpeg",
        });
      }
    ...
    <div className="mx-auto max-w-7xl sm:px-6 lg:px-8 my-10 ">
      ...
      <div className="w-full h-auto bg-[#FCF7F7] rounded-b-lg p-10 ">
        ...
        <div className="flex justify-end mt-4">
          <button
            className="rounded-md bg-todayPink px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-[#FF5284] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 "
            onClick={handleClickEditDiary}
          >
            등록하기
          </button>
        </div>
