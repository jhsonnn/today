...
export const DiaryList = () => {
  ...
  const processContent = (content: string): string => {
    ...
    return cleanContent.length > 20 ? `${cleanContent.slice(0, 100)}...` : cleanContent;
  };
  ...
  return (
    <>
      <ul className="w-full grid grid-cols-4 gap-2 cursor-pointer">
        {diaries.map((diary) => (
          <li className="shadow-md w-[240px] rounded-2xl p-5 bg-white" key={diary.id}>
            <div className="flex justify-between mb-2 items-center relative">
              <span className="w-3 h-3 bg-todayBlue rounded-full"></span>
              <XCircleIcon width={20} className="text-gray-300 cursor-pointer" onClick={() => handleDeleteClick(diary.id)} />
            </div>
            <p className="text-xl font-bold" onClick={() => handleDiaryClick(diary.id)}>{processTitle(diary.title)}</p>
